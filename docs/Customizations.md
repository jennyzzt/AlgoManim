...
## Introduction
While the library generates default animations for every operation you can perform on `AlgoList`, `AlgoBinaryTree` and `AlgoGraph`, the look and feel of the animations may not suit your specific needs and use cases. We thus provide the ability for you to customize the default animations and add additional animations so that you have full control on the final video that is rendered. Furthermore, we separate the functions used to define the customizations (`preconfig` and `customize`) from the main algorithm function (`algo`) thus allowing you to not only animate the same algorithm in multiple different ways, but completely extract the customizations into separate files so that you can present the algorithm code by itself without the customizations affecting code clarity.
...
you can use `AlgoScene`'s `add_transform` method in `def customize(self):`.
...
1. Introduce markers which show which part of the algorithm is being executed in the GUI
2. Add custom animations at these pins later in `customize(self)`.
3. Change default animations (`action_pairs`) around these pins in `customize(self)`.
